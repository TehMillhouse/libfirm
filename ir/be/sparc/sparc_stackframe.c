...
static int sparc_sp_sim(ir_node *const node, int offset)
{
	sparc_determine_frameoffset(node, offset);
	...
	if (is_sparc_Save(node)) {
		...
		int const value
			= -round_up2(-attr->immediate_value, SPARC_STACK_ALIGNMENT);
		attr->immediate_value = value;
