...
static int ia32_sp_sim(ir_node *const node, int offset)
{
	...
	if (is_ia32_Pop(node) || is_ia32_PopMem(node)) {
		...
		offset -= get_mode_size_bytes(ls_mode);
	}

	ia32_determine_frameoffset(node, offset);

	if (is_ia32_Call(node)) {
		offset -= get_ia32_call_attr_const(node)->pop;
...
static void introduce_prologue(ir_graph *const irg, bool omit_fp)
{
	...
	if (!omit_fp) {
		...
		ir_node *incsp = ia32_new_IncSP(block, curr_sp,
		                                frame_size - IA32_REGISTER_SIZE, 0);
